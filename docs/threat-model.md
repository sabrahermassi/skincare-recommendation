...
| Caller IP | used for rate-limiting in `_shared/rate-limit.ts` | PII under GDPR — a dynamic IP can identify a person even without being combined with other logs | Never persisted as an address. Held in-memory per Edge Function isolate, and as a truncated HMAC fingerprint (`fingerprintCaller`) in `rate_limits`, which is service-role only. The raw address reaches Edge Function logs on the request that first crosses the limit — once per caller per window globally when the shared counter refuses, once per isolate when a single isolate spends the whole allowance itself | Same | In-memory: isolate lifetime. Fingerprint: purged hourly, max ~25h. Logs: the platform's own log retention |
