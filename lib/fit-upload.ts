 ...
 * (the server's own refusal still covers that), and `base64` untouched when it
 * already fits or its width is unknown.
 */
export async function fitUpload(sourceUri: string, base64: string, width: number | undefined): Promise<FittedUpload> {
  ...
  for (const step of SHRINK_STEPS) {
    const target = Math.max(MIN_WIDTH, Math.round(width * step.scale));
