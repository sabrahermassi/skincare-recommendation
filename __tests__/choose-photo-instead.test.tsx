...
// #295: "Reading the ingredient list…" appeared the moment the link was
...
describe("ChoosePhotoInstead", () => {
  it("says it is reading only once a photo has been chosen", async () => {
    ...
    expect(screen.getByText("Or choose a photo instead.")).toBeTruthy();
    expect(screen.queryByText("Reading the ingredient list…")).toBeNull();

    await act(async () => resolvePick({ base64: "abc", cleanup: () => {} }));
    expect(screen.getByText("Reading the ingredient list…")).toBeTruthy();
  });
});
